...
export const commandContexts: CommandContext[] = [
  {
    id: 'kafka',
    name: 'Apache Kafka',
    ...
    patterns: [
      {
        pattern: /(?:restart|bounce|reboot)\s+kafka(?:\s+node)?(?:\s+on\s+([^\s]+))?(?:\s+in\s+cluster\s+([^\s]+))?/i,
      ...
      {
        pattern: /(?:restart|bounce|reboot)\s+(?:kafka\s+)?(?:service|datanode)(?:\s+([^\s]+))?(?:\s+on\s+([^\s]+))?(?:\s+in\s+cluster\s+([^\s]+))?(?:\s+(force))?/i,
  ...
  },
  {
    ...
    patterns: [
      {
        pattern: /(?:restart|reboot|bounce)\s+storage(?:\s+node)?(?:\s+on\s+([^\s]+))?(?:\s+rack\s+([^\s]+))?/i,
      ...
      {
        pattern: /(?:clean|wipe|clear|cleanup|free\s+up)\s+(?:disks?|disk\s+space|storage)(?:\s+on\s+([^\s]+))?(?:\s+paths?\s+([^,\s]+(?:,\s*[^,\s]+)*))?(?:\s+(temp_files|log_files|cache|all))?(?:\s+(dry[_\s]?run))?/i,
        intent: 'clean_disks',
        context: 'storage',
        action: 'clean',
  ...
  },
  {
    ...
    patterns: [
      {
        pattern: /(?:restart|bounce|reboot)\s+service\s+([^\s]+)(?:\s+on\s+([^\s]+))?(?:\s+in\s+cluster\s+([^\s]+))?(?:\s+(force))?/i,
        intent: 'restart_service',
        context: 'service',
        action: 'restart',
        parameters: ['service_name', 'hostname', 'cluster', 'force'],
